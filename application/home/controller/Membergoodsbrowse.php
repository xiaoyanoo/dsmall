...
class Membergoodsbrowse extends BaseMember {
    ...
    public function del() {
        ...
        if (trim(input('param.goods_id')) == 'all') {
            ...
            if ($goodsbrowse_model->delGoodsbrowse(array('member_id' => session('member_id')))) {
                $return_arr = array('done' => true);
            } else {
                $return_arr = array('done' => false, 'msg' => lang('ds_common_del_fail'));
            }
        } elseif (intval(input('param.goods_id')) >= 0) {
            $goods_id = intval(input('param.goods_id'));
            if ($goodsbrowse_model->delGoodsbrowse(array('member_id' => session('member_id'), 'goods_id' => $goods_id))) {
                $return_arr = array('done' => true);
            } else {
                $return_arr = array('done' => false, 'msg' =>  lang('ds_common_del_fail'));
            }
