...
class Login extends BaseMall {
    ...
    public function login() {
        ...
        if (!request()->isPost()) {
            ...
            //验证数据  BEGIN
            $rule = [
                ['member_name', 'require', lang('account_required')],
                ['member_password', 'require', lang('password_required')]
            ];
    ...
    public function register() {
        if (!request()->isPost()) {
            ...
            //验证数据  BEGIN
            $rule = [
                ['member_name', 'require|length:3,15', lang('account_required')| lang('account_length_must_limited')],
                ['member_password', 'require|length:6,20', lang('password_required')| lang('password_length_must_limited')]
            ];
            $validate = new Validate($rule);
            $validate_result = $validate->check($data);
            if (!$validate_result) {
