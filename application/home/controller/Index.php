...
class Index extends BaseMall {
    ...
    private function getIndexData()
    {
        ...
        if (empty($index_data)) {
            $index_data = array();
            $index_data['recommend_list'] = model('goods')->getGoodsOnlineList(array('goods_commend' => 1), 'goods_id,goods_name,goods_advword,goods_image,store_id,goods_promotion_price,goods_price', 0, 'rand()',6,'goods_commonid');
            //限时折扣
            $index_data['promotion_list'] = model('pxianshigoods')->getXianshigoodsCommendList(6);
            $index_data['new_list'] = model('goods')->getGoodsOnlineList(array(), '', '', 'goods_addtime desc', 6,'goods_commonid');
            $index_data['groupbuy_list'] = model('groupbuy')->getGroupbuyCommendedList(6);
            //友情链接
    ...
    private function getFloorList($cate_id) {
        ...
        if (empty($result)) {
            ...
            $goods_list[0]['gc_list'] = model('goods')->getGoodsOnlineList(array('gc_id' => $cate_id),'*', 0, 'goods_commend desc,goods_id desc', 8,'goods_commonid');
            ...
            foreach ($hot_goods_class_list as $key => $hot_goods_class) {
                ...
                $data['gc_list'] = model('goods')->getGoodsOnlineList(array('gc_id' => $data['gc_id']),'*', 0, 'goods_commend desc,goods_id desc', 8,'goods_commonid');
                $goods_list[] = $data;
            }
