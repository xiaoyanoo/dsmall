...
class Goods extends BaseGoods {
    ...
    public function index() {
       ...
       // halt($goods_info);
        ...
        //推荐商品 
        $goods_commend_list = $goods_model->getGoodsOnlineList(array('store_id' => $goods_info['store_id'], 'goods_commend' => 1), 'goods_id,goods_name,goods_advword,goods_image,store_id,goods_price', 0, 'rand()', 5, 'goods_commonid');
    ...
    public function save_consult() {
        ...
        $rule = [
            ['goods_content', 'require', lang('goods_index_cosult_not_null')],
        ];
        $res = $ob_validate->check($data, $rule);
        if (!$res) {
            ds_json_encode(10001,$ob_validate->getError());
        }
    ...
    public function arrival_notice_submit() {
        ...
        if (!empty($notice_info)) {
            if ($type == 1) {
                ds_show_dialog(lang('goods_no_repeat_add'), 'reload');
            } else {
                ds_show_dialog(lang('goods_no_repeat_appointment'),'reload');
            }
        }
    ...
    public function arrival_notice_succ() {
        ...
        return $this->fetch($this->template_dir . 'arrival_notice_message');
    }
