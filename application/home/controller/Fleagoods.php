...
class Fleagoods extends BaseFlea {
    ...
    public function save_consult() {
        ...
        if (empty(session('member_id'))) {
            $this->error(lang('flea_consult_notice'));
        }
        /**
         * 判断商品编号的存在性和合法性
         */
        ...
        if (empty($goods_info)) {
            if (input('post.type_name') == '') {
                $this->error(lang('goods_index_goods_not_exists'));
            }
        }
        /**
         * 咨询内容的非空验证
        ...
        if (trim(input('post.content')) === "") {
            $this->error(lang('goods_index_input_consult')); //'请输入咨询内容'
        }
        $member_model = model('member');
        ...
        if ($consult->addFleaconsult($input)) {
            ...
            $this->success(lang('goods_index_consult_success'), url('Fleagoods/index') . '?goods_id=' . intval(input('post.goods_id')) . '&succ=succ'); //'咨询发布成功'
        } else {
            $this->error(lang('goods_index_consult_fail')); //'咨询发布超时'
        }
    }
