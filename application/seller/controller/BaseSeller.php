...
class BaseSeller extends Controller
{
    ...
    {
        parent::_initialize();
        Lang::load(APP_PATH . 'seller/lang/' . config('default_lang') . '/basemember.lang.php');
        Lang::load(APP_PATH . 'seller/lang/' . config('default_lang') . '/baseseller.lang.php');
        //卖家中心模板路径
        $this->template_dir = '/' . strtolower(request()->controller()) . '/';
//        var_dump($this->template_dir);exit;
        if (request()->controller() != 'Sellerlogin') {
            if (!session('member_id')) {
                $this->redirect('seller/Sellerlogin/login');
            }
