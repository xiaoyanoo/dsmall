...
class Goodsbrowse extends Model {
    ...
    public function getGuessLikeGoods($member_id = 0, $shownum = 0) {
        ...
        if ($member_id > 0) {
            if (!config('cache_open')) {//查询数据库
                $browseclass_list = $this->getGoodsbrowseList(array('member_id' => $member_id), 'gc_id',20, 'rand()', 'gc_id'); //随机抽取20条信息
        ...
        //随机显示符合以上分类的推荐展位商品
        $boothgoodslist = model('pbooth')->getBoothgoodsList($where, '', 0, $shownum, 'rand()');
        $goodsid_arr = array();
