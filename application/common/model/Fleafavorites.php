...
class Fleafavorites extends Model {
    ...
    public function addFleafavorites($data) {
        ...
        if (is_array($data)) {
            ...
            $result = db('fleafavorites')->insert($tmp);
            return $result;
        } else {
            return false;
        }
     ...
     * @return boolean
     */
    public function delFleafavorites($id, $type) {
        if (intval($id) > 0 && !empty($type)) {
            $result = db('fleafavorites')->where('fleafav_id', intval($id))->where('member_id', session('member_id'))->where('fleafav_type', $type)->delete();
            return $result;
        } else {
            return false;
        }
