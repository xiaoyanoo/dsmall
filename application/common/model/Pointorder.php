            ...
            if ($prod_list) {
                ...
                foreach ($prod_list as $v) {
                    ...
                    $update_arr['pgoods_storage'] = array(
                        'exp',
                        "pgoods_storage+{$v['pointog_goodsnum']}"
                    );
                    $update_arr['pgoods_salenum'] = array(
                        'exp',
                        "pgoods_salenum-{$v['pointog_goodsnum']}"
                    );
    ...
    public function createOrder($post_arr, $pointprod_arr, $member_info) {
        ...
        if ($pointprod_arr['pointprod_list']) {
            ...
            foreach ($pointprod_arr['pointprod_list'] as $v) {
                ...
                $tmp['pgoods_storage'] = array(
                    'exp',
                    "pgoods_storage-{$v['quantity']}"
                );
                $tmp['pgoods_salenum'] = array(
                    'exp',
                    "pgoods_salenum+{$v['quantity']}"
                );
                $pointprod_uparr[] = $tmp;
                unset($tmp);
            }
