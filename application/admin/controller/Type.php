...
class Type extends AdminControl {
    ...
    public function type_add() {
        if (!(request()->isPost())) {
            ...
            //验证数据  BEGIN
            $rule = [
                ['type_name', 'require', lang('type_add_name_no_null')],
                ['type_sort', 'require|number', lang('type_add_sort_no_null').'|类型排序必须为数字'],
    ...
    public function type_edit() {
        ...
        if (!(request()->isPost())) {
            ...
            //验证数据  BEGIN
            $rule = [
                ['type_name', 'require', '类型名称为必填'],
                ['type_sort', 'require|number', '类型排序为必填|类型排序必须为数字'],
                ['class_id', 'require|number', '分类为必填|分类ID必须为数字'],
            ];
            $validate = new Validate($rule);
            $validate_result = $validate->check($data);
