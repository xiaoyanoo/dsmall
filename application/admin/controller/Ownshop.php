...
class Ownshop extends AdminControl {
    ...
    public function index() {
        ...
        $storesWithGoods = model('goods')->where(array('store_id' => array('in', $store_ids),))->field('distinct store_id')->select();
        $storesWithGoods = ds_change_arraykey($storesWithGoods, 'store_id');
    ...
    public function add() {
        if (!request()->isPost()) {
            ...
            $store_id = $storeModel->addStore($saveArray);
            ...
            //插入店铺扩展表
            $storeModel->addStoreextend(array('store_id' => $store_id));
    ...
    public function del() {
        ...
        $storeModel = model('store');
        $storeArray = $storeModel->getOneStore(array('store_id'=>$store_id),'is_platform_store,store_name');
        if (empty($storeArray)) {
            $this->error('自营店铺不存在');
        }
