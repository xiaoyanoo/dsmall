...
class Consulting extends AdminControl {
    ...
    public function setting() {
        ...
        if (request()->isPost()) {
            $update_array = array();
            $update_array['consult_prompt'] = input('post.consult_prompt');
            $result = $config_model->editConfig($update_array);
            if ($result === true) {
                $this->log(lang('edit_consult_text_tips'), 1);
                $this->success(lang('ds_common_op_succ'));
            } else {
                $this->log(lang('edit_consult_text_tips'), 0);
                $this->error(lang('ds_common_op_fail'));
            }
        }
        $this->setAdminCurItem('setting');
        $setting_list = $config_model->getConfigList();
        /* 所见即所得编辑器 */
